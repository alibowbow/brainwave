...
export const Player: React.FC<PlayerProps> = ({
  sessionName, timeLeft, isPlaying, onPlay, onPause, onStop, onMinimize,
  onTimeChange, currentBrainWave, onWaveChange, currentSound, onSoundChange,
  ...
  return (
    // Removed h-full to allow natural scrolling
    <div className="flex flex-col animate-slide-up pb-32">
      ...
      <div className="text-center mb-8">
        ...
        <div className="text-slate-500 dark:text-slate-400 text-sm flex items-center justify-center gap-2">
           <Activity size={14}/> {currentBrainWave === 'alpha' ? '알파파' : currentBrainWave === 'beta' ? '베타파' : currentBrainWave === 'theta' ? '세타파' : '델타파'}
           <span>•</span>
      ...
      {/* Scrollable Live Settings - Removed internal overflow handling to let parent scroll */}
        ...
        <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm">
          ...
          <div className="grid grid-cols-4 gap-2">
            {(['alpha', 'beta', 'theta', 'delta'] as BrainWaveType[]).map(wave => (
              ...
              >
                {wave === 'alpha' ? 'α 알파' : wave === 'beta' ? 'β 베타' : wave === 'theta' ? 'θ 세타' : 'δ 델타'}
              </button>
            ))}
          </div>
        </div>
