...
const Brand = () => (
  <div className="flex items-center gap-3">
    ...
    <div className="min-w-0 leading-tight">
      <strong className="block font-serif text-[21px] font-semibold tracking-[-0.035em] text-primary-900 dark:text-amber-100">Brainwave</strong>
      <span className="block text-[10px] font-semibold tracking-[0.04em] text-slate-500 dark:text-slate-500">RITUAL SOUND STUDIO</span>
    </div>
  </div>
);
...
export const AppShell: React.FC<Props> = ({
  ...
  return (
    <div className="app-shell min-h-[100dvh] bg-amber-50 text-primary-900 dark:bg-primary-900 dark:text-amber-100">
      <header className="relative z-40 border-b border-slate-200 bg-amber-50/95 dark:border-primary-400/30 dark:bg-primary-900/95">
        <div className="mx-auto flex min-h-[72px] w-full max-w-[1680px] items-center gap-4 px-4 sm:px-6 lg:min-h-[78px] lg:px-8">
                ...
                <button
                  ...
                  className={`flex min-h-11 items-center gap-2 rounded-xl px-3.5 text-xs font-bold transition-colors ${
                    active
                      ...
                      : 'text-slate-500 hover:bg-amber-100 hover:text-primary-900 dark:text-slate-400 dark:hover:bg-white/6 dark:hover:text-amber-100'
                  }`}
                >
          ...
          <div className="ml-auto flex shrink-0 items-center gap-2">
            <div className="hidden min-w-[166px] items-center gap-3 rounded-xl border border-slate-200 bg-white/38 px-3.5 py-2.5 xl:flex dark:border-white/10 dark:bg-white/[0.035]">
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-3 text-[10px] font-bold text-slate-500 dark:text-slate-400">
                  <span>오늘의 진행</span>
                  <span className="tabular-nums text-primary-900 dark:text-amber-100">{dailyMinutes}/{dailyGoalMinutes}분</span>
                </div>
            ...
            <button
              ...
              className="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-white/42 text-slate-500 transition-colors hover:bg-white hover:text-primary-900 dark:border-white/10 dark:bg-white/[0.045] dark:text-slate-400 dark:hover:bg-white/9 dark:hover:text-amber-100"
            >
            ...
            <button
              ...
              className={`grid h-11 w-11 place-items-center rounded-xl border transition-colors ${activeView === 'settings'
                ? 'border-primary-400/30 bg-primary-100 text-primary-900 dark:bg-primary-900 dark:text-primary-100'
                : 'border-slate-200 bg-white/42 text-slate-500 hover:bg-white hover:text-primary-900 dark:border-white/10 dark:bg-white/[0.045] dark:text-slate-400 dark:hover:bg-white/9 dark:hover:text-amber-100'}`}
            >
              <Settings size={17} />
            </button>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 grid h-[calc(64px+env(safe-area-inset-bottom))] grid-cols-3 border-t border-slate-200 bg-amber-50/95 px-3 pb-[env(safe-area-inset-bottom)] backdrop-blur-md md:hidden dark:border-white/10 dark:bg-primary-900/95" aria-label="주요 메뉴">
