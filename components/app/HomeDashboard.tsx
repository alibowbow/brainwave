    ...
    <div className="mx-auto w-full max-w-[1500px] px-4 py-5 sm:px-6 sm:py-7 lg:px-9 lg:py-8">
      <div className="mb-6 flex items-end justify-between gap-4 lg:mb-8">
        <div>
          ...
          <h2 className="mt-1 text-[28px] font-black tracking-[-0.055em] text-slate-950 sm:text-[34px] dark:text-white">
            {getGreeting(now)}
          </h2>
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,0.7fr)]" aria-label="오늘의 추천">
        <article className={`group relative min-h-[390px] overflow-hidden rounded-[30px] bg-gradient-to-br ${visual.gradient} text-white shadow-[0_26px_70px_rgba(18,32,70,0.22)] sm:min-h-[430px]`}>
          {visual.artwork ? (
            <img
          ...
          <div className="relative flex min-h-[390px] flex-col p-6 sm:min-h-[430px] sm:p-9">
            <div className="flex items-center justify-between">
              <span className="rounded-full border border-white/16 bg-white/10 px-3 py-1.5 text-[10px] font-black tracking-[0.14em] backdrop-blur-md">지금 추천</span>
            ...
            <div className="mt-auto max-w-xl">
              <p className="text-xs font-black tracking-[0.14em] text-white/62">{visual.eyebrow}</p>
              <h3 className="mt-2 text-[36px] font-black leading-[1.03] tracking-[-0.06em] sm:text-[52px]">{visual.text}</h3>
              ...
              <div className="mt-6 flex flex-wrap gap-2.5">
                <button
                  ...
                  className="flex min-h-12 items-center gap-2 rounded-full bg-white px-5 text-sm font-black text-slate-950 shadow-xl shadow-black/20 transition-transform hover:scale-[1.02] active:scale-[0.98]"
                >
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
          <article className="rounded-[28px] border border-slate-200/80 bg-white p-5 shadow-sm dark:border-white/8 dark:bg-[#111621]">
            <div className="flex items-center justify-between gap-4">
              <div>
                <p className="text-[10px] font-black tracking-[0.15em] text-slate-400">TODAY</p>
                <h3 className="mt-1 text-lg font-black tracking-[-0.035em] text-slate-950 dark:text-white">오늘의 리듬</h3>
              </div>
              <div
              ...
              >
                <div className="grid h-[66px] w-[66px] place-items-center rounded-full bg-white text-center dark:bg-[#111621]">
                  <div>
                    <strong className="block text-lg font-black tabular-nums text-slate-950 dark:text-white">{todayMinutes}</strong>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2">
              <div className="rounded-2xl bg-slate-50 p-3 dark:bg-white/[0.035]">
                ...
                <strong className="mt-2 block text-lg font-black text-slate-950 dark:text-white">{streak}일</strong>
                <span className="text-[10px] font-semibold text-slate-400">연속 기록</span>
              </div>
              <div className="rounded-2xl bg-slate-50 p-3 dark:bg-white/[0.035]">
                <Sparkles size={15} className="text-violet-400" />
                <strong className="mt-2 block text-lg font-black text-slate-950 dark:text-white">{logs.length}회</strong>
                <span className="text-[10px] font-semibold text-slate-400">완료 세션</span>
              </div>
            </div>
          </article>

          <article className="flex min-h-[184px] flex-col rounded-[28px] border border-slate-200/80 bg-white p-5 shadow-sm dark:border-white/8 dark:bg-[#111621]">
            {lastSession ? (
              <>
                <div className="flex items-start justify-between">
                  ...
                  <span className="text-[10px] font-black tracking-[0.13em] text-slate-400">RECENT</span>
                </div>
                <div className="mt-auto">
                  <h3 className="truncate text-lg font-black tracking-[-0.035em] text-slate-950 dark:text-white">{lastSession.name}</h3>
                  ...
                  <button type="button" onClick={onResumeLast} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 py-3 text-xs font-black text-white transition-transform active:scale-[0.99] dark:bg-white dark:text-slate-950">
              ...
              <div className="flex h-full flex-col items-start justify-center">
                ...
                <h3 className="mt-4 text-lg font-black tracking-[-0.035em] text-slate-950 dark:text-white">첫 루틴을 시작해 보세요</h3>
      ...
      <section className="mt-9" aria-labelledby="quick-title">
        <div className="mb-4 flex items-end justify-between">
          <div>
            <p className="text-[10px] font-black tracking-[0.16em] text-[#7180f0]">QUICK START</p>
            <h3 id="quick-title" className="mt-1 text-xl font-black tracking-[-0.04em] text-slate-950 dark:text-white">지금 필요한 리듬</h3>
          </div>
          ...
          {quickPresets.map((preset, index) => {
            ...
            return (
              <button
                ...
                className="group relative min-h-[156px] overflow-hidden rounded-[24px] bg-slate-900 p-4 text-left text-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl"
              >
                ...
                <div className="relative mt-8">
                  <strong className="block text-sm font-black tracking-[-0.02em]">{itemVisual.text}</strong>
      ...
      <section className="mt-9" aria-labelledby="soundscape-title">
        <div className="mb-4 flex items-end justify-between">
          <div>
            <p className="text-[10px] font-black tracking-[0.16em] text-emerald-500">SOUNDSCAPES</p>
            <h3 id="soundscape-title" className="mt-1 text-xl font-black tracking-[-0.04em] text-slate-950 dark:text-white">소리로 공간 바꾸기</h3>
          </div>
          ...
          {FEATURED_AMBIENCES.map((preset) => {
            ...
            return (
              <button
                ...
                className={`group relative min-h-[154px] overflow-hidden rounded-[24px] bg-gradient-to-br ${itemVisual.gradient} p-5 text-left text-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl`}
              >
                ...
                <div className="relative flex h-full flex-col">
                  ...
                  <div className="mt-auto pt-7">
                    <strong className="block text-base font-black tracking-[-0.025em]">{preset.name}</strong>
