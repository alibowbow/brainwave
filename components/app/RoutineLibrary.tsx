...
export const RoutineLibrary: React.FC<Props> = ({
  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] px-4 py-5 sm:px-6 sm:py-7 lg:px-9 lg:py-8">
      <section className="overflow-hidden rounded-[30px] bg-[#111725] p-6 text-white shadow-[0_28px_80px_rgba(11,18,38,0.2)] sm:p-8 lg:flex lg:items-end lg:justify-between lg:gap-10">
        <div className="max-w-2xl">
          <p className="text-[10px] font-black tracking-[0.18em] text-[#8d9aff]">BUILD YOUR RHYTHM</p>
          <h2 className="mt-2 text-[32px] font-black leading-tight tracking-[-0.055em] sm:text-[42px]">필요한 순간에 바로 꺼내 쓰는 루틴</h2>
          <p className="mt-3 max-w-xl text-sm leading-relaxed text-white/56">집중·이완·수면 목적별 세션과 여러 자연음을 조합한 공간을 한곳에서 찾고 저장하세요.</p>
        </div>
        <button type="button" onClick={onCreateCustom} className="mt-6 flex min-h-12 w-full items-center justify-center gap-2 rounded-full bg-white px-5 text-sm font-black text-slate-950 transition-transform hover:scale-[1.015] active:scale-[0.985] lg:mt-0 lg:w-auto">
      ...
      {savedRoutines.length > 0 ? (
        <section className="mt-8" aria-labelledby="saved-title">
          <div className="mb-3 flex items-center justify-between">
            <div>
              <p className="text-[10px] font-black tracking-[0.16em] text-[#7180f0]">MY ROUTINES</p>
              <h3 id="saved-title" className="mt-1 text-lg font-black tracking-[-0.035em] text-slate-950 dark:text-white">내가 저장한 루틴</h3>
            </div>
            <span className="text-[11px] font-bold text-slate-400">{savedRoutines.length}개</span>
          </div>
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {savedRoutines.map((routine) => (
              <article key={routine.id} className="relative w-[255px] shrink-0 rounded-[24px] border border-slate-200 bg-white p-4 shadow-sm dark:border-white/8 dark:bg-[#111621]">
                ...
                <h4 className="mt-4 truncate pr-7 text-base font-black tracking-[-0.025em] text-slate-950 dark:text-white">{routine.name}</h4>
                ...
                <button type="button" onClick={() => onOpenSaved(routine.id)} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 py-2.5 text-xs font-black text-white dark:bg-white dark:text-slate-950"><ArrowRight size={14} /> 열기</button>
              </article>
            ))}
          </div>
        </section>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide" role="group" aria-label="카테고리 필터">
            {(['all', 'focus', 'calm', 'sleep', 'restore'] as const).map((id) => (
              <button key={id} type="button" onClick={() => setCategory(id)} aria-pressed={category === id} className={`shrink-0 rounded-full px-4 py-2.5 text-xs font-black transition-all ${category === id ? 'bg-slate-950 text-white shadow-lg shadow-slate-950/15 dark:bg-white dark:text-slate-950' : 'border border-slate-200 bg-white text-slate-500 hover:border-slate-300 hover:text-slate-950 dark:border-white/8 dark:bg-white/4 dark:text-slate-400 dark:hover:text-white'}`}>{CATEGORY_LABELS[id]}</button>
            ))}
            <button type="button" onClick={() => setCategory('favorites')} aria-pressed={category === 'favorites'} className={`shrink-0 rounded-full px-4 py-2.5 text-xs font-black transition-all ${category === 'favorites' ? 'bg-rose-500 text-white shadow-lg shadow-rose-500/20' : 'border border-slate-200 bg-white text-slate-500 hover:text-rose-500 dark:border-white/8 dark:bg-white/4 dark:text-slate-400'}`}><Heart size={13} className="mr-1 inline" fill={category === 'favorites' ? 'currentColor' : 'none'} /> 즐겨찾기</button>
          </div>
        </div>
      ...
      {resultCount === 0 ? (
        <div className="mt-8 rounded-[28px] border border-dashed border-slate-300 py-20 text-center dark:border-white/12">
          <Search size={28} className="mx-auto text-slate-300 dark:text-slate-600" />
          <h3 className="mt-4 font-black text-slate-700 dark:text-slate-200">조건에 맞는 루틴이 없어요</h3>
        ...
        <>
          {presetResults.length > 0 ? (
            <section className="mt-7" aria-labelledby="guided-title">
              <div className="mb-4 flex items-end justify-between">
                <div>
                  <p className="text-[10px] font-black tracking-[0.16em] text-[#7180f0]">GUIDED SESSIONS</p>
                  <h3 id="guided-title" className="mt-1 text-xl font-black tracking-[-0.04em] text-slate-950 dark:text-white">목적별 세션</h3>
                </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                {presetResults.map((preset) => {
                  ...
                  return (
                    <article key={preset.id} className="content-auto group overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-950/8 dark:border-white/8 dark:bg-[#111621]">
                      <button type="button" onClick={() => onOpenPreset(preset)} className={`relative block aspect-[16/8.8] w-full overflow-hidden bg-gradient-to-br ${visual.gradient} text-left`}>
                        ...
                        <span className="absolute bottom-4 left-4 rounded-full bg-black/28 px-3 py-1.5 text-[10px] font-black text-white backdrop-blur-md">{visual.eyebrow}</span>
                        <span className="absolute bottom-4 right-4 rounded-full bg-white/92 px-3 py-1.5 text-[10px] font-black text-slate-950">{preset.defaultDurationMinutes}분</span>
                      </button>
                      <div className="p-5">
                        <div className="flex items-start justify-between gap-3">
                          <button type="button" onClick={() => onOpenPreset(preset)} className="min-w-0 text-left">
                            <h4 className="truncate text-lg font-black tracking-[-0.035em] text-slate-950 dark:text-white">{visual.text}</h4>
                        ...
                        <div className="mt-4 flex gap-2">
                          <button type="button" onClick={() => onQuickStartPreset(preset)} className="flex min-h-11 flex-1 items-center justify-center gap-2 rounded-2xl bg-slate-950 text-xs font-black text-white transition-transform active:scale-[0.98] dark:bg-white dark:text-slate-950"><Play size={14} fill="currentColor" /> 바로 시작</button>
          ...
          {ambienceResults.length > 0 ? (
            <section className="mt-10" aria-labelledby="layered-title">
              <div className="mb-4">
                <p className="text-[10px] font-black tracking-[0.16em] text-emerald-500">LAYERED SOUNDSCAPES</p>
                <h3 id="layered-title" className="mt-1 text-xl font-black tracking-[-0.04em] text-slate-950 dark:text-white">공간형 사운드스케이프</h3>
              </div>
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {ambienceResults.map((preset) => {
                  ...
                  const favorite = favoriteIds.has(favoriteKey);
                  return (
                    <article key={preset.id} className={`content-auto relative min-h-[210px] overflow-hidden rounded-[26px] bg-gradient-to-br ${visual.gradient} p-5 text-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl`}>
                      ...
                      <div className="relative flex min-h-[170px] flex-col">
                        ...
                        <button type="button" onClick={() => onOpenAmbience(preset)} className="mt-auto text-left">
                          <h4 className="text-lg font-black tracking-[-0.035em]">{preset.name}</h4>
                          <p className="mt-1 line-clamp-2 text-xs leading-relaxed text-white/62">{preset.description}</p>
                        </button>
                        <div className="mt-4 flex items-center justify-between gap-3">
                          <span className="flex items-center gap-2 text-[10px] font-black text-white/60"><Layers3 size={13} /> {preset.layers.length}개 <Clock3 size={13} /> {preset.durationMinutes}분</span>
